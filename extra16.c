  ...
  for (i = 0; i < l+10; i++) {
    ...
    if (c == 0) {
      ...
      if (c == l) {
        /*
          Vector está lleno:
          Si valor es el mayor, se agrega al final y se corren
          los números hacia atras.
          6 → 1 2 4 5 6 → 2 4 5 6 6 (el 1 es eliminado)
          Si valor es menor, se corren los números hacia delante
          y se cambia el valor por el número que es mayor a él.
          4 → 2 4 5 6 6 → 2 4 4 5 6 (el 6 es eliminado)
        */
        for (j = 0; j < c; j++) {
          if (valor <= vector[j] && valor != vector[c-1]) {
            ...
            // Se ejecuta si el valor es mayor o igual
            for (k = 0; k < j; k++) {
              vector[k] = vector[k+1];
            }
            vector[j] = valor;
        ...
        /*
          Vector NO está lleno: (Ninguno es eliminado)
